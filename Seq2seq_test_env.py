...
def train(model, optimizer, Tensor, train_dataloader, val_dataloader, test_dataloader, loss_fun, decode_dic):
    epoch = 2
    model.train()
    #training steps
    max_bleu = 0
    ...
    for e in range(epoch):
        ...
        #validation steps
        if e > 0:
            val_results, bleu, hit, com = eval_set(model, val_dataloader, decode_dic)
            print(val_results[0:10])
    ...
    print('Best Val BLEU:%f, HIT:%f, COMMON:%f' %(save_file['best_bleu'], save_file['best_hit'], save_file['best_common']))
    model.load_state_dict(save_file_best['para'])
    test_results, bleu, hit, com = eval_set(model, test_dataloader, decode_dic)
...
ans = []
for t in range(2):
    seq2seq, optimizer, Tensor, train_dataloader, val_set, test_set, loss_fun, config = build(768, 9, 50, True)
    bleu, hit, com = train(seq2seq, optimizer, Tensor, train_dataloader, val_set, test_set, loss_fun, config)
