...
from dcmn_seq2seq_v2.dcmn import BertForMultipleChoiceWithMatch
from dcmn_seq2seq_v2.preprocess import get_dataloader
from dcmn_seq2seq_v2.train import train_valid
from dcmn_seq2seq_v2.config import DCMN_Config

from dcmn_seq2seq_v2.Seq2seq import DecoderRNN, Seq2seq, SEP, CLS
from dcmn_seq2seq_v2.draw import DataGenerator
import dcmn_seq2seq_v2.models.bert as seq_bert
...
def build_dcmn():
    ...
    train_dataloader, train_examples_size = get_dataloader(data_dir=dcmn_config.data_dir,
                                                           data_file=dcmn_config.train_file,
                                                           num_choices=dcmn_config.num_choices,
                                                           tokenizer=tokenizer,
                                                           max_seq_length=dcmn_config.max_seq_length,
    ...
    scheduler = get_linear_schedule_with_warmup(optimizer,
                                                num_warmup_steps=int(
                                                    dcmn_config.warmup_proportion * dcmn_config.t_total),
...
def build_seq2seq(config, hidden_size, max_len, no_cuda, dg):
    ...
    scheduler = get_linear_schedule_with_warmup(optimizer,
                                                num_warmup_steps=int(
                                                    config.warmup_proportion * len(
                                                        dg.train_src_txt) * config.num_epochs),
                                                num_training_steps=len(
                                                    dg.train_src_txt) * config.num_epochs)  # PyTorch scheduler
...
def main():
    ...
    seq2seq, seq_optimizer, seq_scheduler, seq_loss_fun = build_seq2seq(seq_config, 768, dcmn_config.max_seq_length,
                                                                        dcmn_config.no_cuda, dg)
    save_file_best = torch.load('./cache/best_save.data', map_location=torch.device('cuda:1'))
