...
from utils import build_iterator, get_time_dif, build_iterator_eval
from dataset_step3 import build_dataset, build_dataset_eval
...
class Seq2seq(nn.Module):
    ...
    def forward(self, batch_src, indexes, batch_sum_embs, batch_tar=None,
                teacher_forcing_ratio=0):
            
        encoder_outputs, encoder_hidden = self.encoder(batch_src)

        for i, src in enumerate(batch_src[0]):
            p = 0
            for j, u in enumerate(indexes[i]):
                encoder_outputs[i][u].data = torch.cuda.FloatTensor(batch_sum_embs[i][p]).data
                p += 1
...
with open('./data_tmp/train_preds123.pkl', 'rb') as f:
    train_preds = pickle.load(f)
with open('./data_tmp/test_preds123.pkl', 'rb') as f:
    test_preds = pickle.load(f)

# with open('./data_tmp/train_word_scores.pkl', 'rb') as f:
#     train_preds = pickle.load(f)
# with open('./data_tmp/test_word_scores.pkl', 'rb') as f:
#     test_preds = pickle.load(f)
train_sum_embs = []
test_sum_embs = []

def eval_set(model, dataloader, config, dcmn, dataloader_test, dcmn_trainer, dcmn_loss_func, test_embs, e,ctx, test_index):
    global test_sum_embs

    if len(test_sum_embs) == 0:
        # class_weight=None
        # loss_name='sce'
        # is_train = False
        # preds, dcmn_loss = train_dcmn.one_epoch(dataloader_test, dcmn, dcmn_loss_func, dcmn_trainer, ctx, is_train, \
        #           e, class_weight, loss_name)
        preds  = test_preds
        scores = []
        for output in preds:
            # print(output)
            score = output.asnumpy()
            # score = output
            scores.append(score)
        ...
        for embs in test_embs:
            ...
            if p >= len(scores):
                break
            for avg_embs in embs:
                if p >= len(scores):
                    break
                new_embs = [sc*emb for sc, emb in zip(scores[p], avg_embs)]
                sum_new_embs.append(np.sum(new_embs, axis=0))
                p+=1
            test_sum_embs.append(sum_new_embs)
    ...
    for i, (batch_src, train_tar, train_dic) in enumerate(dataloader):
        replace_embs = test_sum_embs[p:p + config.batch_size]
        p += config.batch_size
...
def get_index(srcs):
    ...
    for i, src in enumerate(srcs):
        ...
        for j, u in enumerate(src):
            if u == 4:
                indexs.append((i%16,j))
        tar_indexs.append(indexs)
    return tar_indexs

def train(seq2seq, optimizer, Tensor, train_dataloader, val_dataloader, test_dataloader, seq_loss_fun, config, \
         dcmn, dataloader_train, dataloader_test, dcmn_trainer, dcmn_loss_func, ctx):
    global train_sum_embs
    train_embs = pickle.load(open('./data_tmp/train_embs.pkl','rb'))
    test_embs = pickle.load(open('./data_tmp/test_embs.pkl','rb'))
    train_src = pickle.load(open("./data_tmp/train_src_23.pkl", 'rb'))
    test_src = pickle.load(open("./data_tmp/test_src_23.pkl", 'rb'))
    train_index = get_index(train_src)
    test_index = get_index(test_src)
    print('start train')
    ...
    for e in range(config.num_epochs):
        ...
        if len(train_sum_embs) == 0:
            # train
            # is_train = False
            # preds, dcmn_loss = train_dcmn.one_epoch(dataloader_train, dcmn, dcmn_loss_func, dcmn_trainer, ctx, is_train, \
            #           e, class_weight, loss_name)
            preds = train_preds
            scores = []
            for output in preds:
            ...
            for embs in train_embs:
                ...
                for avg_embs in embs:
                    ...
                    sum_new_embs.append(np.sum(new_embs, axis=0))
                    p+=1
                train_sum_embs.append(sum_new_embs)
        
        p = 0
        for i, (batch_src, batch_tar) in tqdm(enumerate(train_dataloader)):
            replace_embs = train_sum_embs[p:p+config.batch_size]
            p += config.batch_size

            decoder_outputs, decoder_hidden, ret_dict = seq2seq(batch_src, train_index[i*config.batch_size: (i+1)*config.batch_size],replace_embs, batch_tar[0], 0.5)
            optimizer.zero_grad()
            ...
            loss = (seq_loss_fun(input=logit, target=target) * mask).sum() / mask.sum()
            # loss += dcmn_loss*10
            loss.backward()
            optimizer.step()
            if i % 50 == 0:
        ...
        #validation steps
        if e >= 0:
            val_results, bleu, hit, com, ascore = eval_set(seq2seq, val_dataloader, config, dcmn, dataloader_test, dcmn_trainer, dcmn_loss_func, test_embs, e, ctx, test_index)
            ...
            if bleu > max_bleu:
                ...
                # dcmn.save_parameters('./cache/dcmn-best.params')
    ...
    # dcmn.load_parameters('./cache/dcmn-best.params')
    test_results, bleu, hit, com, ascore = eval_set(seq2seq, val_dataloader, config, dcmn, dataloader_test, dcmn_trainer, dcmn_loss_func, test_embs, e, ctx,test_index)
...
def start_exp():
    ...
    for t in range(1):
        # ctx = mx.cpu()
        seq2seq, optimizer, Tensor, train_dataloader, val_set, test_set, seq_loss_fun, config = build(768, 16, 64, True)
        # dcmn, dataloader_train, dataloader_test, dcmn_trainer, dcmn_loss_func = build_dcmn(64, 64, ctx)
        # save_file_best = torch.load('./epoch30-opt/cache/best_save.data')
        # seq2seq.load_state_dict(save_file_best['para'])
        # dcmn.load_parameters('./cache/dcmn-best-1.params')
        dcmn, dataloader_train, dataloader_test, dcmn_trainer, dcmn_loss_func, ctx = None, None, None, None, None, None
        bleu, hit, com = train(seq2seq, optimizer, Tensor, train_dataloader, val_set, test_set, seq_loss_fun, config, \
                              dcmn, dataloader_train, dataloader_test, dcmn_trainer, dcmn_loss_func, ctx)
